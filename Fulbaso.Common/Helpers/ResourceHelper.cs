...
namespace Fulbaso.Common
{
    public abstract class ResourceHelper<TRawString> where TRawString : class
    {
        ...
        internal string GetStringResource(string key)
        {
            ...
            if (!string.IsNullOrEmpty(viewPath)) // expecting: "~/Views/User/LogOn.cshtml" or "Account/Register"
            {
        ...
        internal string GetSharedResource(string file, string key)
        {
            ...
            string resourceTypeName = string.Format("{0}.{1}.Resources.{2}, {3}", namespaceRoot, GetSharedResourceNamespace(), file, assemblyName);
        ...
        internal string GetStringResource(Type resourceType, string resourceName, bool throwIfNotFound = false)
        {
            ...
            if (property.PropertyType != typeof(string))
            {
                if (throwIfNotFound)
                {
                    throw new InvalidOperationException("Resource " + resourceName + " is not a string.");
                }
                return resourceName;
            }
