...
namespace Fulbaso.EntityFramework.BusinessLogic
{
    internal static class Repository<T> where T : EntityObject
    {
        ...
        internal static IEnumerable<K> GetFromQuery<K>(IQueryable<T> query) where K : EntityDataObject
        {
            return query.ToList().Select(i => i.ToEntity<K>());
        }

        internal static IEnumerable<K> Get<K>(Expression<Func<T, bool>> predicate) where K : EntityDataObject
        {
            return GetFromQuery<K>(objectSet.Where(predicate));
        }

        internal static IEnumerable<EntityDataObject> Get(Expression<Func<T, bool>> predicate)
        {
            return Get<EntityDataObject>(predicate);
        }
